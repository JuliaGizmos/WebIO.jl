...
export const createWebIOEventListener = (
    _webIOElement: WebIODomElement,
    _webIOListenerSource: string | EventListener,
    _webIOScope?: WebIOScope,
): EventListener => {
  log("Creating event listener.", {element: _webIOElement, scope: _webIOScope, source: _webIOListenerSource});
  if (typeof _webIOListenerSource === "string") {
    ...
    // declaration).
    return (eval(`(${_webIOListenerSource})`) as EventListener).bind(_webIOElement);
  }
